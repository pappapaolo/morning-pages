...
const FlameIcon = ({ size = 'small', className = '', isGrey = false }) => {
  const dimensions = size === 'large' ? { width: 80, height: 100 } : { width: 20, height: 25 };
  const gradientId = `flameGradient-${size}${isGrey ? '-grey' : ''}`;
  ...
  return (
    <svg
      ...
      className={`flame-icon ${isGrey ? 'flame-grey' : ''} ${className}`}
    >
      <defs>
        {isGrey ? (
